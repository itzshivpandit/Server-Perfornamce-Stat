...
get_top_cpu_processes() {
    ...
    ps -eo pid,user,%cpu,cmd --sort=-%cpu | awk 'NR<=6 {printf "PID: %-6s User: %-8s CPU: %-5s CMD: %s\n", $1, $2, $3, substr($0, index($0, $4))}'
}

# Function to get top 5 processes by memory usage (Full Command)
get_top_memory_processes() {
    echo "--- Top 5 Processes by Memory Usage ---"
    ps -eo pid,user,%mem,cmd --sort=-%mem | awk 'NR<=6 {printf "PID: %-6s User: %-8s MEM: %-5s CMD: %s\n", $1, $2, $3, substr($0, index($0, $4))}'
}
...
get_failed_logins() {
    ...
    if command -v journalctl &>/dev/null; then
        ...
        failed_logins=$(lastb --since yesterday | wc -l)
    fi
    echo "$failed_logins"
}
